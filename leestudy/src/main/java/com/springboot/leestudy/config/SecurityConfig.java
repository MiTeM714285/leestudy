...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		.antMatchers("/auth/**") // 이 uri들은
		.authenticated() // 인증 거치기
		.anyRequest() // 나머지 요청들은
		.permitAll() // 인증 필요없음
		.and()
		.formLogin() // 파라미터를 받아서 로그인
		.loginPage("/login") // 로그인 페이지 get요청(view)
		.loginProcessingUrl("/login") // 로그인 post요청(PrincipalDetailsService -> loadUserByUsername() 호출)
		.defaultSuccessUrl("/auth/search") // 로그인성공시 (학생 및 선생 판별후 알맞는 사이트로 리다이렉트됨)
